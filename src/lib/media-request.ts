...
function resolveImageForProtocol(modelId: string | undefined, fallback = DEFAULT_MODEL_ID): ImageModel {
  ...
  if (isStrictImageProviderAlias(candidate) || isStrictImageProviderAlias(legacy)) {
    ...
    if (/^gpt[-_]image[-_]2\.5[-_]flare$/i.test(legacy)) return resolveImageModel("gpt-image-2.5-flare-1k-16x9");
    if (/^gpt[-_]image[-_]2\.5[-_](sunburst|prism)$/i.test(legacy)) return resolveImageModel("gpt-image-2.5-sunburst-1k-16x9");
    if (/^gpt[-_]image[-_]2\.5$/i.test(legacy)) return resolveImageModel("gpt-image-2.5-flare-1k-16x9");
